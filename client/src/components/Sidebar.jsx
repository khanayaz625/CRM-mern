...
import { LayoutDashboard, Users, LogOut, X, Settings, BarChart3, ChevronLeft, ChevronRight, Plus, BookOpen, Globe } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const SidebarContent = ({ onClose, onLogout, user, isCollapsed, setSidebarCollapsed, onAddLead }) => {
    const location = useLocation();

    const menuItems = [
        { path: '/dashboard', label: 'Dashboard', icon: LayoutDashboard },
        { path: '/manage-leads', label: 'Manage Leads', icon: Users },
        { path: '/leads/by-course', label: 'By Course', icon: BookOpen },
        { path: '/leads/by-college', label: 'By College', icon: Globe },
        { path: '/course-analysis', label: 'Course Analysis', icon: BarChart3 },
    ];

    if (user?.role === 'admin') {
        menuItems.push({ path: '/team-members', label: 'Team Members', icon: Users });
    }

    // Profile is always there
    menuItems.push({ path: '/profile', label: 'My Profile', icon: Settings });
            ...
            <div className={`p-4 flex-1 overflow-y-auto ${isCollapsed ? 'px-2' : ''}`}>
                ...
                <div className={`mb-6 flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/5 overflow-hidden transition-all duration-300 ${isCollapsed ? 'justify-center px-0 bg-transparent border-transparent' : ''}`}>
                    <div className={`rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold overflow-hidden border border-white/10 shrink-0 transition-all duration-300 ${isCollapsed ? 'w-10 h-10' : 'w-10 h-10'}`}>
                        {user?.avatar ? (
                            <img src={user.avatar.startsWith('http') ? user.avatar : `${import.meta.env.VITE_API_URL}${user.avatar}`} alt="" className="w-full h-full object-cover" />
                        ) : (
                ...
                <nav className="space-y-2">
                    {menuItems.map((item) => (
                        <NavLink
                            ...
                            to={item.path}
                            onClick={onClose}
                            className={({ isActive }) => `w-full flex items-center gap-3 rounded-xl transition-all duration-300 ${isCollapsed ? 'justify-center p-3' : 'px-4 py-3'} ${isActive
                                ? 'bg-white/10 text-white border border-white/10 shadow-lg' // Distinct active state
...
const Sidebar = ({ isOpen, onClose, onLogout, user, isCollapsed, setSidebarCollapsed, onAddLead }) => {
    return (
        <>
            ...
            <AnimatePresence>
                {isOpen && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="md:hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-40" />
                        <motion.div initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="md:hidden fixed top-0 left-0 h-full z-50 shadow-2xl w-64">
                            <SidebarContent
                                onClose={onClose}
                                onLogout={onLogout}
                                user={user}
