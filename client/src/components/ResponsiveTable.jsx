...
const ResponsiveTable = ({
    ...
    onPrint, onExportPDF, onExportXLSX, onAssignLead, disableActions = false
}) => {
        ...
        <div className={`p-4 bg-surface border ${selectedLeads?.includes(lead._id) ? 'border-primary' : 'border-white/5'} rounded-xl mb-4 space-y-4 shadow-lg transition-colors`}>
            <div className="flex justify-between items-start">
                <div className="flex items-start gap-3">
                    {user?.role === 'admin' && !disableActions && (
                        <input
                            type="checkbox"
            ...
            {!disableActions && (
                <div className="hidden md:flex p-6 border-b border-white/5 justify-between items-center bg-white/5">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                        Recent Leads
                        {selectedLeads && selectedLeads.length > 0 && <span className="text-sm font-normal text-primary bg-primary/10 px-3 py-1 rounded-full">{selectedLeads.length} Selected</span>}
                    </h2>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-white/10 text-sm uppercase text-gray-400">
                            ...
                            {!disableActions && <th className="p-4 font-semibold tracking-wider text-right px-6">Actions</th>}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                        {loading ? (
                            ...
                            <tr key={lead._id} className={`hover:bg-white/5 transition-colors group ${selectedLeads && selectedLeads.includes(lead._id) ? 'bg-primary/5' : ''}`}>
                                {user?.role === 'admin' && !disableActions && (
                                    <td className="p-4">
                                        <input
                                            type="checkbox"
                                ...
                                <td className="p-4 max-w-xs">
                                    {/* User requested: If remarks are optional, show "No remarks added" */}
                                    {lead.notes ? <p className="text-xs text-gray-400 line-clamp-2" title={lead.notes}>{lead.notes}</p> : <span className="text-xs text-muted italic">No remarks added</span>}
                                </td>
                                <td className="p-4">
                                    {disableActions ? (
                                        <div className={`text-[10px] uppercase tracking-widest font-black px-3 py-1.5 rounded-full inline-block border ${getStatusColor(lead.status)}`}>{lead.status}</div>
                                    ) : (
                                        <select
                                            value={lead.status}
                                            onChange={(e) => updateStatus(lead._id, e.target.value)}
